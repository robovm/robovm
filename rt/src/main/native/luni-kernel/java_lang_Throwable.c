...
jlong Java_java_lang_Throwable_fillInStackTrace0(Env* env, Object* thiz) {
...
ObjectArray* Java_java_lang_Throwable_getStackTraceImpl(Env* env, Object* thiz, jlong state) {
    Class* java_lang_StackTraceElement = nvmFindClass(env, "java/lang/StackTraceElement");
    if (!java_lang_StackTraceElement) return NULL;

    jint length = 0;
    CallStackEntry* first = (CallStackEntry*) state;
    while (first) {
        length++;
        first = first->next;
    }
