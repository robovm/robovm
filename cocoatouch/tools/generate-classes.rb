...
class ObjCEnum
  ...
  def to_java
    ...
    s += "    private final long n;\n\n"
    s += "    private #{@name}(long n) { this.n = n; }\n"
    s += "    public long value() { return n; }\n"
    s += "    public static #{@name} fromValue(long n) {\n"
    s += "        for (#{@name} v : values()) {\n"
    s += "            if (n == v.value()) {\n"
    s += "                return v;\n"
    s += "            }\n"
    ...
    if (@conf[@name] || {})['bitmask']
      s += "    public static EnumSet<#{@name}> fromBits(long bits) {\n"
      s += "        EnumSet<#{@name}> set = EnumSet.noneOf(#{@name}.class);\n"
      s += "        for (#{@name} v : values()) {\n"
      s += "            long value = v.value();\n"
      ...
      s += "        return set;\n"
      s += "    }\n"
      s += "    public static long toBits(EnumSet<#{@name}> set) {\n"
      s += "        long bits = 0;\n"
      s += "        for (#{@name} v : set) {\n"
      s += "            bits |= v.value();\n"
      s += "        }\n"
