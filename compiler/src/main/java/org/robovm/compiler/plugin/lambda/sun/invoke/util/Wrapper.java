...
import org.robovm.compiler.plugin.lambda.*;

public enum Wrapper {
    BOOLEAN(SClass.BooleanClass, SClass.booleanClass, 'Z', (Boolean)false, new boolean[0], Format.unsigned(1)),
    // These must be in the order defined for widening primitive conversions in JLS 5.1.2
    BYTE(SClass.ByteClass, SClass.byteClass, 'B', (Byte)(byte)0, new byte[0], Format.signed(8)),
    SHORT(SClass.ShortClass, SClass.shortClass, 'S', (Short)(short)0, new short[0], Format.signed(16)),
    CHAR(SClass.CharacterClass, SClass.charClass, 'C', (Character)(char)0, new char[0], Format.unsigned(16)),
    INT(SClass.IntegerClass, SClass.intClass, 'I', (Integer)/*(int)*/0, new int[0], Format.signed(32)),
    LONG(SClass.LongClass, SClass.longClass, 'J', (Long)(long)0, new long[0], Format.signed(64)),
    FLOAT(SClass.FloatClass, SClass.floatClass, 'F', (Float)(float)0, new float[0], Format.floating(32)),
    DOUBLE(SClass.DoubleClass, SClass.doubleClass, 'D', (Double)(double)0, new double[0], Format.floating(64)),
    //NULL(Null.class, null.class, 'N', null, null, Format.other(1)),
    OBJECT(SClass.ObjectClass, SClass.ObjectClass, 'L', null, new Object[0], Format.other(1)),
    // VOID must be the last type, since it is "assignable" from any other type:
    VOID(SClass.VoidClass, SClass.voidClass, 'V', null, null, Format.other(0)),
    ;

    private final SClass<?> wrapperType;
    private final SClass<?> primitiveType;
    ...
    private Wrapper(SClass<?> wtype, SClass<?> ptype, char tchar, Object zero, Object emptyArray, int format) {
...
//    }
     ...
     */
    public static Wrapper forPrimitiveType(SClass<?> type) {
    ...
    static Wrapper findPrimitiveType(SClass<?> type) {
     ...
     *  @throws IllegalArgumentException for unexpected types
     */
    public static Wrapper forWrapperType(SClass<?> type) {
    ...
    static Wrapper findWrapperType(SClass<?> type) {
     ...
     */
    public static Wrapper forBasicType(SClass<?> type) {
        if (type.isPrimitive())
            return forPrimitiveType(type);
    ...
    private static int hashPrim(SClass<?> x) {
        String xn = x.getName();
        if (xn.length() < 3)  return 0;
        return (xn.charAt(0) + xn.charAt(2)) % 16;
    }
    private static int hashWrap(SClass<?> x) {
        String xn = x.getName();
    ...
    public SClass<?> primitiveType() { return primitiveType; }

    /** What is the wrapper type for this wrapper? */
    public SClass<?> wrapperType() { return wrapperType; }
     ...
     */
    public <T> SClass<T> wrapperType(SClass<T> exampleType) {
        if (exampleType == wrapperType) {
            return exampleType;
        } else if (exampleType == primitiveType ||
                   wrapperType == SClass.ObjectClass ||
    ...
    private static ClassCastException newClassCastException(SClass<?> actual, SClass<?> expected) {
     ...
     */
    public static <T> SClass<T> asWrapperType(SClass<T> type) {
        if (type.isPrimitive()) {
     ...
     */
    public static <T> SClass<T> asPrimitiveType(SClass<T> type) {
        Wrapper w = findWrapperType(type);
        if (w != null) {
     ...
     */
    public static char basicTypeChar(SClass<?> type) {
        if (!type.isPrimitive())
            return 'L';
        else
...
//    private <T> T convert(Object x, SClass<T> type, boolean isCast) {
//        if (this == OBJECT) {
...
//            return result;
//        }
//        SClass<T> wtype = wrapperType(type);
//        if (wtype.isInstance(x)) {
//            return wtype.cast(x);
//        }
//        if (!isCast) {
//            SClass<?> sourceType = x.getSClass();  // throw NPE if x is null
...
//        assert (result == null ? Void.class : result.getSClass()) == wtype;
//        return result;
//    }
     ...
     * If the target type is a primitive, change it to a wrapper.
     */
    static <T> SClass<T> forceType(SClass<?> type, SClass<T> exampleType) {
        boolean z = (type == exampleType ||
               ...
               type == SClass.ObjectClass && !exampleType.isPrimitive());
        if (!z)
        ...
        assert(type == exampleType ||
               ...
               type == SClass.ObjectClass && !exampleType.isPrimitive());
        @SuppressWarnings("unchecked")
        SClass<T> result = (SClass<T>) type;  // unchecked warning is expected here
        return result;
    }
