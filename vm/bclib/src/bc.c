...
// Alternative startup method when using robovm to create static library instead of full app
int initAsLib(int argc, char* argv[])
{
	for (int i = 0; i < argc; ++i) {
		fprintf(stderr, "bclib.initAsLib: (%i) - (%s)  \n", i, argv[i]);
	}
	// This dummy class must be provided in the Java source of the static library
	options.mainClass = "org/robovm/staticlibrary/RoboVmMain"; // (char*) _bcMainClass;
	...
	vm = env->vm;
	jint result = rvmRunAsLib(env) ? 0 : 1;
	fprintf(stderr, "bclib.initAsLib: result(%d)\n", result);
...
static Class* ldcClass(Env* env, ClassInfoHeader* header)
{
	...
	if (!clazz) {
		...
		clazz = rvmFindClassUsingLoader(env, header->className, loader);
		if (!clazz)
			wrapClassNotFoundException(env, header->className);
	}
	return clazz;
}
