...
public class ProjectProperties {
    ...
    private void setArch(String arch) {
        ...
        if (arch == null) {
            archCombo.select(0);
        } else if (arch.equals(NullVMPlugin.ARCH_AUTO)) {
            archCombo.select(1);
        } else {
    ...
    private void setOs(String os) {
        ...
        if (os == null) {
            osCombo.select(0);
        } else if (os.equals(NullVMPlugin.OS_AUTO)) {
            osCombo.select(1);
        } else {
    ...
    protected void createControls(Composite parent) {
        ...
        prefsLink.setText("<a>Configure default NullVM settings...</a>");
        ...
        prefsLink.addSelectionListener(new SelectionAdapter() {
            ...
            @Override
            public void widgetSelected(SelectionEvent e) {
                String id = NullVMPreferencePage.ID;
                PreferencesUtil.createPreferenceDialogOn(NullVMPlugin.getShell(), id, new String[] { id }, null).open();
            }
        });
    }
    ...
    public void loadPreferences(IProject project) {
        IEclipsePreferences node = new ProjectScope(project).getNode(NullVMPlugin.PLUGIN_ID);
        setArch(node.get(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_ARCH, null));
        setOs(node.get(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_OS, null));
    }
    
    public void storePreferences(IProject project) throws BackingStoreException {
        IEclipsePreferences node = new ProjectScope(project).getNode(NullVMPlugin.PLUGIN_ID);
        if (arch != null) {
            node.put(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_ARCH, arch);
        } else {
            node.remove(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_ARCH);
        }
        if (os != null) {
            node.put(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_OS, os);
        } else {
            node.remove(NullVMPlugin.PREFERENCE_INCREMENTAL_BUILD_OS);
        }
        node.flush();
    }
}
