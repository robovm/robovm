...
import static org.robovm.compiler.plugin.lambda.sun.invoke.util.Wrapper.forPrimitiveType;
import static org.robovm.compiler.plugin.lambda.sun.invoke.util.Wrapper.forWrapperType;
import static org.robovm.compiler.plugin.lambda.sun.invoke.util.Wrapper.isWrapperType;
...
/* package */ abstract class AbstractValidatingLambdaMetafactory {
     ...
     * }}
     */
    final SClass<?> targetClass;               // The class calling the meta-factory via invokedynamic "class X"
    final SMethodType invokedType;             // The type of the invoked method "(CC)II"
    final SClass<?> samBase;                   // The type of the returned instance "interface JJ"
    final String samMethodName;               // Name of the SAM method "foo"
    final SMethodType samMethodType;           // Type of the SAM method "(Object)Object"
    final SMethodHandle implMethod;            // Raw method handle for the implementation method
    final SMethodHandleInfo implInfo;          // Info about the implementation method handle "MethodHandleInfo[5 CC.impl(int)String]"
    ...
    final SClass<?> implDefiningClass;         // Type defining the implementation "class CC"
    final SMethodType implMethodType;          // Type of the implementation method "(int)String"
    final SMethodType instantiatedMethodType;  // Instantiated erased functional interface method type "(Integer)Object"
    final boolean isSerializable;             // Should the returned instance be serializable
    final SClass<?>[] markerInterfaces;        // Additional marker interfaces to be implemented
    final SMethodType[] additionalBridges;     // Signatures of additional methods to bridge
     ...
     */
    AbstractValidatingLambdaMetafactory(SMethodHandles.Lookup caller,
                                       SMethodType invokedType,
                                       String samMethodName,
                                       SMethodType samMethodType,
                                       SMethodHandle implMethod,
                                       SMethodType instantiatedMethodType,
                                       boolean isSerializable,
                                       SClass<?>[] markerInterfaces,
                                       SMethodType[] additionalBridges)
...
//        }
        ...
        this.implIsInstanceMethod =
                implKind == SMethodHandleInfo.REF_invokeVirtual ||
                implKind == SMethodHandleInfo.REF_invokeSpecial ||
                implKind == SMethodHandleInfo.REF_invokeInterface;
     ...
     */
    abstract SCallSite buildCallSite()
            throws LambdaConversionException;
    ...
    void validateMetafactoryArgs() throws LambdaConversionException {
        ...
        SClass<?> actualReturnType =
                (implKind == SMethodHandleInfo.REF_newInvokeSpecial)
                  ? implDefiningClass
