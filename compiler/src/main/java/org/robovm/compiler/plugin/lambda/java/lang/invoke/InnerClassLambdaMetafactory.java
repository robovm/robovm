...
//import org.robovm.compiler.plugin.lambda.sun.misc.Unsafe;
//import org.robovm.compiler.plugin.lambda.sun.security.action.GetPropertyAction;
 ...
 */
/* package */ public class InnerClassLambdaMetafactory extends AbstractValidatingLambdaMetafactory {
    //private static final Unsafe UNSAFE = Unsafe.getUnsafe();

    private static final int CLASSFILE_VERSION = /*52*/51;
...
//        String path = null;/*AccessController.doPrivileged(
...
//    }
    ...
    private final SClass<?> implMethodReturnClass;    // class for implementaion method return type "Ljava/lang/String;"
    private final SMethodType constructorType;        // Generated class constructor type "(CC)void"
     ...
     */
    public InnerClassLambdaMetafactory(SMethodHandles.Lookup caller,
        ...
        implMethodReturnClass = (implKind == SMethodHandleInfo.REF_newInvokeSpecial)
                ? implDefiningClass
    ...
    SCallSite buildCallSite() throws LambdaConversionException {
        byte[] classData = spinInnerClass();
        if (invokedType.parameterCount() == 0) {
            return new SCallSite(lambdaClassName, classData, "<init>", invokedType);
        } else {
            return new SCallSite(lambdaClassName, classData, NAME_FACTORY, invokedType);
        }
...
//        }
     ...
     * is not found
     */
    public /*Class<?>*/ byte[] spinInnerClass() throws LambdaConversionException {
        String[] interfaces;
...
//        return UNSAFE.defineAnonymousClass(targetClass, classBytes, null);
    ...
    private void generateConstructor() {
        ...
        MethodVisitor ctor = cw.visitMethod(/*ACC_PRIVATE*/0, NAME_CTOR,
        ...
        for (int i = 0, lvIndex = 0; i < parameterCount; i++) {
            ctor.visitVarInsn(ALOAD, 0);
            SClass<?> argType = invokedType.parameterType(i);
    ...
    private void generateSerializationFriendlyMethods() {
        ...
        mv.visitInsn(DUP);
        mv.visitLdcInsn(Type.getType(targetClass.getDescriptor()));
    ...
    private class ForwardingMethodGenerator extends TypeConvertingMethodAdapter {
        ...
        void generate(SMethodType methodType) {
            ...
            if (implKind == SMethodHandleInfo.REF_newInvokeSpecial) {
    ...
    static int getParameterSize(SClass<?> c) {
        if (c == SClass.voidClass) {
            return 0;
        } else if (c == SClass.longClass || c == SClass.doubleClass) {
            return 2;
        }
        return 1;
    }
    ...
    private static int getOpcodeOffset(SClass<?> c) {
        if (c.isPrimitive()) {
            if (c == SClass.longClass) {
                return 1;
            } else if (c == SClass.floatClass) {
                return 2;
            } else if (c == SClass.doubleClass) {
                return 3;
            }
            return 0;
        } else {
