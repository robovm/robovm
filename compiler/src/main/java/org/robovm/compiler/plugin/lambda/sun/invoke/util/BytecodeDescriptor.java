...
//    public static List<SClass<?>> parseMethod(String bytecodeSignature, SClassLoader loader) {
//        return parseMethod(bytecodeSignature, 0, bytecodeSignature.length(), loader);
//    }

//    static List<SClass<?>> parseMethod(String bytecodeSignature,
//            int start, int end, SClassLoader loader) {
//        if (loader == null)
//            loader = SClassLoader.getSystemClassLoader();
//        String str = bytecodeSignature;
//        int[] i = {start};
//        ArrayList<SClass<?>> ptypes = new ArrayList<SClass<?>>();
//        if (i[0] < end && str.charAt(i[0]) == '(') {
//            ++i[0];  // skip '('
//            while (i[0] < end && str.charAt(i[0]) != ')') {
//                SClass<?> pt = parseSig(str, i, end, loader);
//                if (pt == null || pt == SClass.voidClass)
...
//        }
//        SClass<?> rtype = parseSig(str, i, end, loader);
...
//    static private SClass<?> parseSig(String str, int[] i, int end, SClassLoader loader) {
...
//            }
//        } else if (c == '[') {
//            SClass<?> t = parseSig(str, i, end, loader);
//            if (t != null)
...
//    }
    ...
    public static String unparse(Object type) {
        if (type instanceof SClass<?>)
            return unparse((SClass<?>) type);
        if (type instanceof SMethodType)
            return unparse((SMethodType) type);
        return (String) type;
    }
    ...
    static private void unparseSig(SClass<?> t, StringBuilder sb) {
